...
execute unless data storage km_solver:_ {disable_error_messages:1b} if data storage km_solver:_ {depend_check:0b} run tellraw @a {"text":"エラー：KMSolverの前提ライブラリKMFloatOpが見つかりませんでした。\nhttps://github.com/komaramune/KMFloatOp から導入してください。\nこのメッセージをクリックするとリンクが開きます。","clickEvent":{"action":"open_url","value":"https://github.com/komaramune/KMFloatOp"}}